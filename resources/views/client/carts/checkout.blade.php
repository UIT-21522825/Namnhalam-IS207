                ...
                <div class="card border-secondary mb-5">
                    ...
                    <div class="card-body">
                        ...
                        @if (session('discount_amount_price'))
                            <div class="d-flex justify-content-between">
                                <h6 class="font-weight-medium">Giá giảm Coupon </h6>
...
@section('script')
    <script>
        $(function() {
            ...
            // Bắt sự kiện khi form được submit (khi nút thanh toán được nhấn)
        $('form').submit(function(event) {
            // Ngăn chặn hành động mặc định của form (không cho form submit thông qua HTTP request)
            event.preventDefault();

            // Lấy đường dẫn action của form (đây có thể là "{{ route('client.checkout.proccess') }}" trong trường hợp này)
            var formAction = $(this).attr('action');

            // Thực hiện submit form (gửi request)
            $.ajax({
                type: 'POST',
                url: formAction,
